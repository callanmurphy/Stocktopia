...
// Stock object
class Stock {
    constructor(ticker, exchange, info = randomData()){
        this.ticker = ticker;
...
function priceChangeFormat(num, type) {
    if (num < 0){
        if (type === "%"){
...
function createStats(stock) {
    ...
    stats[0] = priceChangeFormat(data[data.length - 1] - data[0]);           // $ Change
    stats[1] = priceChangeFormat((data[data.length - 1] / data[0] - 1)*100, "%"); // % Change
    stats[2] = "$" + data[0];                         // Open
...
function displayTable(stock, id, colour) {
...
function displayChart(stock, id, colour) {
    ...
    for(i in stock.prices) {
        price = parseInt(stock.prices[i] - low + 50);
        // console.log(price);
        ctx.beginPath();
